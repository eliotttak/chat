  ...
  if (typeof data == "string") {
    try {
      ...
      for (key in dataAsObj) {
        console.log(
          `    - ${dispJSONKey(key)} : ${key === "value" && dataAsObj.type.startsWith("message") ? dispMessage(dataAsObj[key]) : dispJSONValue(dataAsObj[key])}`,
        );
      }
      console.log("\n\n");
    } catch (e) {
...
const dispURL = (url) => colors.bgYellow(colors.underline(url));
...
console.log(
  ...
  dispMessage("message"),
  dispURL("URL"),
  dispJSONKey("JSON key"),
  dispJSONValue("JSON value"),
  dispUndefined(),
);

const localIPv4 = (function () {
  ...
  fs.readFile(
    ...
    (err, data) => {
      if (err) {
        ...
        if (
          request.url.split(".")[request.url.split(".").length - 1] == "html"
        ) {
...
server.listen(httpPort, "", () => {
  console.log(
    `Bienvenue sur un chat en ligne à l'adresse '${dispURL(`http://${localIPv4}:${httpPort}`)}' ( [ctrl] + clic gauche )\n`,
  );
});
    ...
    connection.wsConnection.on("message", (msg) => {
      try {
        ...
        switch (usableMsg.type) {
          case "newPseudo":
            const verifiedPseudo = usableMsg.value
              .replaceAll("<", "&lt;")
              .replaceAll("\n", "");
            ...
            for (let user of connections) {
              if (
                user.pseudo === verifiedPseudo ||
                verifiedPseudo === "server"
              ) {
                // Check if there is already a user with the same pseudo.
            ...
            connection.pseudo =
              connection.pseudo === undefined
                ? verifiedPseudo
                : connection.pseudo; // Modify the pseudo only if it is not saved
