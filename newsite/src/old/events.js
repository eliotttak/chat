  ...
  if (/messageToClients?/.test(valueObject.type)) {
    ...
    if (valueObject.sender == "server") {
      document.getElementById("messages").innerHTML +=
        `<span class="fromServer">${valueObject.content}</span><br/><br />`;
    } else {
      document.getElementById("messages").innerHTML +=
        `<span class="pseudoLabel">&nbsp;${valueObject.sender} </span>&nbsp;<span class="message">&nbsp;${valueObject.content.replaceAll("<br />", "&nbsp;<br />&nbsp;")}&nbsp;</span><br/><br />`;

      if (!document.hasFocus() && valueObject.sender !== pseudo) {
        new Notification(
          "Chat - Vous avez reçu un nouveau message de " + valueObject.sender,
          {
            lang: "fr",
            badge: "/favicon.ico",
            icon: "/favicon.ico",
            body:
              valueObject.content.length <= 80
                ? valueObject.content
                : valueObject.content.substring(0, 80) + "…",
          },
        );
      }
    }
    ...
    switch (valueObject.value) {
      ...
      case technicalMessages.fromServer.errors.incorrectPseudo:
        ...
        console.log(
          "%cCe pseudo est incorrect. Merci de recommencer.",
